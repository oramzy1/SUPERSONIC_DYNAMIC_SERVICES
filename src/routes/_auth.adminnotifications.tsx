...
export const Route = createFileRoute("/_auth/adminnotifications")({
...
function RouteComponent() {
  ...
  const toggleSelectOne = (id: string, e: React.ChangeEvent<HTMLInputElement>) => {
    e.stopPropagation();
    setSelectedIds((prev) =>
  ...
  return (
    <div className="w-full min-h-screen text-slate-100 px-4 sm:px-6 md:px-8 py-6 font-sans relative overflow-x-hidden">
      {/* PAGE HEADER */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-5 border-b border-[#1c1e21] pb-6 mb-6">
        <div className="space-y-1">
          <h1 className="text-lg sm:text-xl md:text-2xl font-bold text-white flex items-center gap-2.5 leading-tight">
            <Bell className="w-5 sm:w-6 h-6 text-[#E2A54A] shrink-0" />
        ...
        <div className="flex gap-3 sm:self-start md:self-auto shrink-0">
          <div className="bg-[#16191c] border border-slate-800/60 rounded-xl px-4 py-2 text-center min-w-20 flex-1 sm:flex-none">
          ...
          <div className="bg-[#16191c] border border-slate-800/60 rounded-xl px-4 py-2 text-center min-w-20 flex-1 sm:flex-none">
            ...
            <span className="text-base sm:text-lg font-bold text-[#E2A54A]">
      ...
      {/* FILTER & SEARCH ACTIONS CONTROLS */}
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6">
        <div className="w-full lg:w-auto overflow-x-auto bg-[#16191c]/60 p-1 rounded-xl border border-slate-800/40 shrink-0 scrollbar-none">
          <div className="flex items-center gap-1 min-w-max">
            {(["all", "unread", "alerts", "messages"] as const).map((tab) => (
              <button
                ...
                className={`px-3.5 py-1.5 sm:px-4 sm:py-2 text-xs font-semibold capitalize rounded-lg transition-all whitespace-nowrap ${
                  activeTab === tab
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3 w-full lg:max-w-2xl justify-end">
          <div className="flex items-center gap-2.5 bg-[#16191c] border border-slate-800/50 rounded-xl px-3 py-2 w-full focus-within:border-[#E2A54A]/60 transition-colors">
            <Search className="w-4 h-4 text-slate-500 shrink-0" />
            <input
              type="text"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              placeholder="Filter by keyword, title, name..."
          ...
          {selectedIds.length > 0 && (
            <div className="flex items-center gap-2 bg-[#E2A54A]/5 border border-[#E2A54A]/20 px-3 py-1.5 rounded-xl w-full sm:w-auto shrink-0 justify-between sm:justify-start">
      ...
      <div className="flex flex-col lg:flex-row items-start gap-6 transition-all duration-300">
        ...
        <div
          className={`w-full transition-all duration-300 bg-[#16191c]/40 border border-[#1c1e21] rounded-2xl overflow-hidden shadow-2xl backdrop-blur-md ${
            inspectingItem ? "lg:w-2/3" : "lg:w-full"
          }`}
        >
          <div className="overflow-x-auto w-full">
            <table className="w-full text-left border-collapse table-auto min-w-175 lg:min-w-0">
              <thead>
                <tr className="border-b border-[#1c1e21] bg-[#16191c]/80 text-slate-400 text-[11px] uppercase font-bold tracking-wider select-none">
                  ...
                  <th className="py-4 px-3 w-28">Channel</th>
                  <th className="py-4 px-4 w-40">Sender</th>
                    ...
                    <tr
                      ...
                      className={`group hover:bg-[#1c1e21]/40 transition-colors duration-100 cursor-pointer border-l-2 select-none ${
                        item.isRead
                          ...
                          : "border-[#E2A54A] bg-[#E2A54A]/5"
                      } ${inspectingItem?.id === item.id ? "bg-[#1c1e21]/60 text-white" : ""}`}
                    >
                      ...
                      <td className="py-3.5 px-4 font-semibold text-white truncate max-w-120">
                        <div className="flex items-center gap-2">
                          {!item.isRead && (
                      ...
                      <td className="py-3.5 px-4 min-w-50">
                        <div className="flex flex-col gap-0.5">
                          <div className="flex items-center gap-2 flex-wrap">
                            <span
                              className={`font-semibold truncate max-w-45 sm:max-w-none ${!item.isRead ? "text-white" : "text-slate-300"}`}
                            >
                              {item.title}
                            </span>
                            {item.metaId && (
                          ...
                          <p className="text-xs text-slate-400 truncate max-w-60 md:max-w-md">
                            {item.preview}
                          </p>
                        </div>
                      </td>
        ...
        {/* SIDE-BY-SIDE INTEGRATED DETAIL PANEL */}
        {inspectingItem && (
          <div className="w-full lg:w-1/3 bg-[#16191c] border border-slate-800/80 rounded-2xl shadow-2xl overflow-hidden self-start sticky top-6 animate-in fade-in slide-in-from-right-4 duration-200">
            <div className="h-full flex flex-col">
              {/* PANEL HEADER */}
              <div className="flex items-center justify-between px-5 py-4 border-b border-slate-800/60 bg-[#1c1e21]/40">
                <div className="flex flex-col gap-0.5">
                  <span className="text-[10px] font-mono text-slate-500">
                    ID Reference: {inspectingItem.id}
                  </span>
                  <h2 className="text-sm font-semibold text-white">Transmission Details</h2>
                </div>
                <button
                  type="button"
              ...
              {/* PANEL BODY CONTENT */}
              <div className="p-5 space-y-5 overflow-y-auto max-h-[calc(100vh-280px)] lg:max-h-none">
                <div className="bg-[#111315] border border-slate-800/50 rounded-xl p-4 space-y-3">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="bg-[#111315]/60 border border-slate-800/40 px-3.5 py-2.5 rounded-xl text-xs sm:text-sm font-semibold text-white">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="bg-[#111315]/60 border border-slate-800/40 p-4 rounded-xl text-xs text-slate-300 leading-relaxed font-sans min-h-24 whitespace-pre-wrap">
                ...
                {inspectingItem.metaId && (
                  <div className="space-y-1.5">
                    ...
                    <div className="flex items-center justify-between bg-slate-900/60 border border-slate-800/80 px-3.5 py-2.5 rounded-xl gap-2">
                      ...
                      <button
                        ...
                        className="text-[10px] font-bold text-[#E2A54A] hover:underline shrink-0"
                      >
                        Inspect File Record →
                      </button>
                    </div>
              ...
              {/* PANEL ACTION FOOTER */}
              <div className="p-4 bg-[#1c1e21]/40 border-t border-slate-800/60 flex items-center justify-end gap-2">
                <button
                  type="button"
                  onClick={() => {
                    setNotifications((prev) => prev.filter((n) => n.id !== inspectingItem.id));
                    setInspectingItem(null);
                  }}
                  className="flex items-center gap-1.5 px-3 py-2 bg-rose-500/5 hover:bg-rose-500/10 text-rose-400 border border-rose-500/10 rounded-xl text-xs font-semibold transition-colors"
                >
                  <Trash2 className="w-3.5 h-3.5" />
                  Purge
                </button>
