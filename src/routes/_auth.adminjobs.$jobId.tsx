...
function JobDetailPage() {
    ...
    <div className="w-full text-slate-200 select-none pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
         {!completedJob ? ( <div className="bg-[#0d111a]/40 backdrop-blur-md border border-white/6 rounded-xl p-6">
  ...
  <p className="text-xs text-slate-500 mb-5">
    This job was carried out as scheduled. Reopen it from the status controls above if changes are needed.
  </p>
  ...
  <div className="space-y-2">
    ...
    <div className="max-h-56 overflow-y-auto space-y-1.5 pr-1">
      {(job.crew_members ?? []).length === 0 ? (
        <p className="text-xs text-slate-600 py-3 text-center">No crew was recorded on this job.</p>
      ) : (
        job.crew_members.map((member) => (
          ...
          >
            ...
            <span className="text-xs font-medium text-slate-200">{member.full_name}</span>
          </div>
        ))
      )}
    </div>
