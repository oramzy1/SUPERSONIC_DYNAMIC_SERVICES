...
export const Route = createFileRoute("/adminsettings")({
  component: AdminSettingsDashboard,
  validateSearch: (search: Record<string, unknown>): SettingsSearch => {
    return {
      tab: (search.tab as SettingsTab) || "profile",
    };
  },
});

export function AdminSettingsDashboard() {
  const { tab } = useSearch({ from: "/adminsettings" });
  const navigate = useNavigate({ from: "/adminsettings" });
  ...
  const [apiConfig, setApiConfig] = useState({
    ...
    secretKey: import.meta.env.VITE_STRIPE_KEY_SECRET,
  ...
  // Clean clipboard copy state transitions
  useEffect(() => {
    if (copiedKey) {
  ...
  const regenerateApiKeys = () => {
    if (
      ...
      setApiConfig((prev) => ({
        ...prev,
        secretKey: `sk_test_${generatedHex}`,
      }));
  ...
  return (
    <div className="flex h-screen w-full bg-[#07090e] text-[#626d7c] font-sans antialiased overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full relative">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-8 max-w-6xl w-full mx-auto h-[calc(100vh-64px)] space-y-6">
          {/* Dashboard Header Panel */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-[#161b22]">
            <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 items-start">
            {/* Navigation Tabs Matrix Menu */}
            <div className="flex flex-col space-y-1 bg-[#0c1017] p-2 rounded-xl border border-[#161b22]">
              <button
                type="button"
              ...
              >
                <Bell className="h-4 w-4" /> Routing Alerts
              </button>
              <button
                type="button"
              ...
              >
                <Cpu className="h-4 w-4" /> API Interfacing
              </button>
            </div>

            {/* Input Frame Sub-Panels Component */}
            <div className="md:col-span-3 bg-[#0c1017] border border-[#161b22] rounded-xl p-6 min-h-105">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-5 p-4 bg-[#07090e]/50 border border-[#161b22] rounded-xl">
                      ...
                      <div className="flex flex-col gap-1.5 items-center sm:items-start text-center sm:text-left">
                        <label className="cursor-pointer bg-[#16191c] hover:bg-slate-800 border border-slate-800 text-slate-200 text-xs font-semibold px-3 py-1.75 rounded-lg transition flex items-center gap-2">
                        ...
                        <p className="text-[10px] text-slate-500">
                          Supports RAW images, PNG, or JPEG metadata up to 2MB limits.
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="flex items-center justify-between p-4 bg-[#07090e] border border-[#161b22] rounded-xl gap-4">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[11px] text-slate-500 mt-0.5">
                            Enforce hardware/TOTP cryptographic challenges on core operations
                            Resets.
                          </p>
                        </div>
                      </div>
                      <label className="relative inline-flex items-center cursor-pointer shrink-0 select-none">
                        ...
                        />
                        <div className="w-9 h-5 bg-[#161b22] border border-slate-800 rounded-full peer peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-[#626d7c] peer-checked:after:bg-[#07090e] after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-[#e2a54a] peer-checked:border-transparent"></div>
                      </label>
                    </div>
                ...
                {activeTab === "notifications" && (
                  <div className="space-y-6">
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between p-3.5 bg-[#07090e]/60 rounded-lg border border-[#161b22] gap-4">
                        <div className="min-w-0">
                          <p className="text-xs font-bold text-white">
                            Critical Telemetry Violations
                          </p>
                          <p className="text-[10px] text-slate-500 mt-0.5">
                            Broadcast active warnings when metrics breach threshold zones.
                          </p>
                        </div>
                        <input
                          type="checkbox"
                          ...
                          onChange={(e) =>
                            setNotifications({ ...notifications, alertCritical: e.target.checked })
                          }
                      ...
                      <div className="flex items-center justify-between p-3.5 bg-[#07090e]/60 rounded-lg border border-[#161b22] gap-4">
                        <div className="min-w-0">
                          ...
                          <p className="text-[10px] text-slate-500 mt-0.5">
                            Receive pushes when micro-nodes disconnect for maintenance pipelines.
                          </p>
                        </div>
                        <input
                          type="checkbox"
                          checked={notifications.alertMaintenance}
                          onChange={(e) =>
                            setNotifications({
                              ...notifications,
                              alertMaintenance: e.target.checked,
                            })
                          }
                          className="accent-[#e2a54a] h-4 w-4 rounded bg-[#07090e] border-[#161b22] shrink-0 cursor-pointer"
                        />
                      ...
                      <div className="flex items-center justify-between p-3.5 bg-[#07090e]/60 rounded-lg border border-[#161b22] gap-4">
                        <div className="min-w-0">
                          <p className="text-xs font-bold text-white">
                            Weekly Fleet Compilation Matrix
                          </p>
                          <p className="text-[10px] text-slate-500 mt-0.5">
                            Compile logistical optimization summaries into encrypted emails.
                          </p>
                        </div>
                        <input
                          type="checkbox"
