...
export const Route = createFileRoute('/adminservices')({
...
const INITIAL_SERVICES: ServiceDetail[] = [
  {
    ...
    intro: "At SUPERSONIC DYNAMIC SERVICES, we redefine student and micro-moving with a seamless, technology-driven and eco-responsible approach. Our mission is simple: to make your move smooth, efficient, and completely stress-free.",
    body: [
    ...
    expertise: "Private and family household relocations, Senior and elderly home transitions, Care home and assisted living moves, Full or partial house clearances.",
  ...
  },
  {
    ...
    intro: "At SUPERSONIC DYNAMIC SERVICES, we provide comprehensive residential moving solutions across the Netherlands - designed for private individuals, family households, seniors, elderly relocations, care home transfers, and full house clearances.",
    body: [
  ...
  },
  {
    ...
    intro: "SUPERSONIC DYNAMIC SERVICES, is a trusted partner for small, medium, large-scale enterprise and commercial relocations across the Netherlands.",
    body: [
    ...
    expertise: "Corporate hub transitions, Multi-floor workplace relocations, Document file management compliance mapping",
    tags: ["Tech Innovation", "Eco-Centric", "Global Scale"],
    benefits: [
  ...
  },
  {
    ...
    intro: "At SUPERSONIC DYNAMIC SERVICES, we provide safe, flexible, and professionally managed storage solutions tailored for individuals, families, and businesses that need reliable short-term or long-term storage.",
    body: [
    ...
    expertise: "Short-term and long-term storage solutions for household goods, business and commercial goods, furniture and appliances, temporary storage during relocation or renovation.",
    tags: ["Tech Innovation", "Eco-Centric", "Global Scale"],
    benefits: [
  ...
  },
  {
    ...
    intro: "AT SUPERSONIC DYNAMIC SERVICES, we provide fast, efficient, and eco-responsible waste removal services across our regions of operation - helping households and businesses clear unwanted items quickly and responsibly.",
    body: [
    ...
    expertise: "Household waste removal, Furniture and bulky item disposal, Renovation and construction debris removal, Garden and outdoor waste removal, Office and commercial clean-outs, Full property and estate clearances.",
    tags: ["Tech Innovation", "Eco-Centric", "Global Scale"],
    benefits: [
...
function RouteComponent() {
  ...
  const handleInputChange = (field: keyof ServiceDetail, value: any) => {
    ...
    setEditingService(prev => {
      ...
      // Auto-generate standard formatting for slug string if building a new item
  ...
  const removeArrayItem = (field: 'body' | 'tags' | 'benefits', index: number) => {
    if (!editingService) return
    handleInputChange(field, editingService[field].filter((_, i) => i !== index))
  }
  ...
  const saveServiceModifications = (e: React.FormEvent) => {
    ...
    if (isNewService) {
      // Validate unique constraint on slug parameters to prevent index corruption
  ...
  return (
    <div className="flex h-screen w-full bg-[#0d0f11] overflow-hidden font-sans antialiased text-slate-200">
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-6 md:p-8 select-none relative">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div>
              ...
              <p className="text-sm text-slate-400 mt-1">
                Manage available logistics services, configure pricing parameters, and control operational status across all regions.
              </p>
            </div>
            
            <button 
              onClick={openNewServiceModal}
              className="flex items-center gap-1.5 px-4 py-2 bg-[#E2A54A] text-slate-950 font-bold text-xs rounded-lg hover:bg-[#d4963b] transition duration-200"
            >
                  ...
                  <div className="h-48 w-full overflow-hidden relative bg-slate-900/60">
                    {service.image ? (
                      ...
                      <div className="w-full h-full flex flex-col items-center justify-center bg-slate-950/40 border-b border-slate-900 text-slate-600">
                        ...
                        <span className="text-[10px] uppercase tracking-widest font-mono">No Image Attached</span>
                      </div>
                    )}
                    <div className="absolute inset-0 bg-linear-to-t from-[#111315] via-transparent to-transparent" />
                  </div>

                  <div className="p-5 flex-1 flex flex-col justify-between">
                    <div>
                      <h3 className="text-sm font-bold text-white tracking-tight line-clamp-1">{service.title || "Untitled Service module"}</h3>
                      <p className="text-xs text-slate-500 mt-1.5 line-clamp-2 leading-relaxed">{service.intro || "No current summary description payload provided."}</p>
                    </div>
                    ...
                    <div className="flex items-center justify-between mt-6 pt-4 border-t border-slate-800/40">
                      ...
                      <div 
                        ...
                        className={`w-11 h-6 rounded-full p-0.5 cursor-pointer flex items-center transition-colors duration-200 ${
                          isActive ? 'bg-[#E2A54A]/20 border border-[#E2A54A]/30' : 'bg-slate-800 border border-slate-700/50'
                        }`}
                      >
                        <div 
                        ...
                        >
                          {isActive && (
                            <span className="text-[9px] font-extrabold text-slate-950 select-none">✓</span>
                          )}
                        </div>
                      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-slate-800/40 pt-6">
            <div className="bg-[#111315]/40 border border-slate-800/40 rounded-xl p-4 flex items-start gap-3">
              ...
              <div>
                <span className="text-xs font-bold text-white tracking-tight">Active Edge-Caching Node Sync</span>
                <p className="text-[11px] text-slate-500 mt-0.5">Content updates transmitted using custom hooks trigger instant invalidation across client edge routers.</p>
              </div>
            </div>
            <div className="bg-[#111315]/40 border border-slate-800/40 rounded-xl p-4 flex items-start gap-3">
              <Sparkles className="w-4 h-4 text-emerald-400 mt-0.5" />
              <div>
                <span className="text-xs font-bold text-white tracking-tight">Eco-Centric Logistics Metric Flag</span>
                <p className="text-[11px] text-slate-500 mt-0.5">All updated service records automatically inherit Dutch corporate environmental standard tags.</p>
              </div>
            </div>
          </div>

          {/* ========================================== */}
          {/* POPUP CARD CUSTOMIZER DIALOG OVERLAY PANEL */}
          {/* ========================================== */}
          {editingService && (
            <div className="fixed inset-0 z-50 bg-[#070809]/80 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
              <div className="bg-[#111315] border border-slate-800 rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl animate-in fade-in-50 zoom-in-95 duration-150">
                
                {/* Overlay Title shifts dynamically */}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-slate-400 font-mono mt-0.5">
                        {isNewService ? "Target Routing Key: auto-assigned" : `slug: ${editingService.slug}`}
                      </p>
                    </div>
                  </div>
                  <button 
                ...
                <form onSubmit={saveServiceModifications} className="flex-1 overflow-y-auto p-6 space-y-5 text-xs">
                  
                  {/* Service Title Input */}
                  <div className="flex flex-col gap-1.5">
                    <label className="font-bold text-slate-400 uppercase tracking-wider text-[10px]">Card & Display Title</label>
                    <input 
                      type="text" 
                  ...
                  {/* Slug Read-only / Reference block preview for verification mapping */}
                  {isNewService && (
                    <div className="flex flex-col gap-1.5 opacity-75">
                      <label className="font-bold text-slate-500 uppercase tracking-wider text-[10px]">Generated URL Key Route</label>
                      <input 
                        type="text" 
                  ...
                  {/* Hero Title Field */}
                  <div className="flex flex-col gap-1.5">
                    <label className="font-bold text-slate-400 uppercase tracking-wider text-[10px]">Hero Title Segment</label>
                    <input 
                      type="text" 
                  ...
                  {/* Asset Path Selector Fallback */}
                  {isNewService && (
                    <div className="flex flex-col gap-1.5">
                      <label className="font-bold text-slate-400 uppercase tracking-wider text-[10px]">Image Asset Category Preset</label>
                      <select 
                  ...
                  {/* Intro Summary Paragraph Area */}
                  <div className="flex flex-col gap-1.5">
                    <label className="font-bold text-slate-400 uppercase tracking-wider text-[10px]">Intro Text Blocks</label>
                    <textarea 
                      rows={3}
                  ...
                  {/* Expertise Segment Area Input */}
                  <div className="flex flex-col gap-1.5">
                    <label className="font-bold text-slate-400 uppercase tracking-wider text-[10px]">Expertise Scope Matrix</label>
                    <textarea 
                      rows={2}
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <label className="font-bold text-slate-400 uppercase tracking-wider text-[10px]">Detailed Body Paragraph Blocks</label>
                      <button 
                        type="button"
                  ...
                  {/* Meta Tag Badge Arrays */}
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <label className="font-bold text-slate-400 uppercase tracking-wider text-[10px]">Classification Tags</label>
                      <button 
                        type="button"
                  ...
                  {/* Benefits Array Dynamic Content Stack Rows */}
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <label className="font-bold text-slate-400 uppercase tracking-wider text-[10px]">Service Value Benefits & Deliverables</label>
                      <button 
                        type="button"
                  ...
                  <div className="pt-4 border-t border-slate-800 flex items-center justify-end gap-3">
                    ...
                    >
                      <Save className="w-3.5 h-3.5" /> {isNewService ? "Deploy & Post Service" : "Save Changes & Post"}
                    </button>
                  </div>
