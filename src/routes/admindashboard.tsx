...
} from "@/components/admin/AdminDashboardSidebar";
import {
  FileText,
  Truck,
  CheckCircle2,
...
function RouteComponent() {
  const [isAuthenticated, setIsAuthenticated] = useState<boolean>(() => {
    if (typeof window !== "undefined") {
      return localStorage.getItem("supersonic_admin_authed") === "true";
    }
    return false;
  });

  // Safe synchronization utility function
  const syncAuthState = () => {
    if (typeof window !== "undefined") {
      const isAuthed = localStorage.getItem("supersonic_admin_authed") === "true";
      setIsAuthenticated(isAuthed);
    }
  };

  useEffect(() => {
    // Only listen for dynamic state resets across windows or un-focus changes
  ...
  // Return guard interface directly if verification isn't established
  if (!isAuthenticated) {
    return (
      <AdminAuthGuard
        onSuccess={() => {
  ...
  return (
    <div className="flex h-screen w-full bg-[#0d0f11] overflow-hidden font-sans antialiased">
      <AdminDashboardSidebar />

      <div className="flex flex-1 flex-col overflow-hidden">
        <AdminDashboardTopbar />

        <main className="flex-1 overflow-y-auto p-6 md:p-8 text-slate-200 select-none">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div>
              <h1 className="text-2xl font-bold tracking-tight text-white">Overview</h1>
              <p className="text-sm text-slate-400 mt-1">
                System status and live operational metrics.
              </p>
            </div>

            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2 px-3 py-1.5 bg-[#111315] border border-slate-800 rounded-full text-xs font-medium text-emerald-400">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
            {metrics.map((item, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="bg-[#111315] border border-slate-800/60 rounded-xl p-4 flex flex-col justify-between group hover:border-slate-700 transition duration-200"
                >
                  <div className="flex items-center justify-between w-full">
                    <div className="p-2 bg-slate-900/40 rounded-lg border border-slate-800/40 text-[#E2A54A]">
          ...
          <div className="bg-[#111315] border border-slate-800/60 rounded-xl overflow-hidden mb-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-[#111315] border border-slate-800/60 rounded-xl p-6 flex flex-col justify-between">
            ...
            {/* Revenue Analytics */}
            <div className="bg-[#111315] border border-slate-800/60 rounded-xl p-6 flex flex-col justify-between relative overflow-hidden">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-base font-semibold text-white tracking-tight">
                  Revenue Analytics
                </h2>
                <div className="flex items-center gap-1 bg-[#0d0f11] p-1 rounded-lg border border-slate-800/80 text-[10px] font-bold uppercase tracking-wider">
